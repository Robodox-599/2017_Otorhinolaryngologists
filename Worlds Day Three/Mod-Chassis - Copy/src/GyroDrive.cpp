...
bool GyroDrive::straightDrive()
{
	...
	else if((calculateError() > GYRO_ERROR_DEADZONE && calculateError() < GYRO_ERROR_LIMIT) || (calculateError() < -GYRO_ERROR_DEADZONE && calculateError() > -GYRO_ERROR_LIMIT) || (gDrive->abs(navX->GetRate()) > 1))
	{
