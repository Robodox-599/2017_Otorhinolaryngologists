...
void Autonomous::linearAuto3()
{
	...
	while(!(drive->getCANTalon()->GetEncPosition() >= 4388 || drive->getCANTalonLeft()->GetEncPosition() >= 4388) || !time->HasPeriodPassed(1))
	{
		drive->drive(0,0);
		drive->addForwardSpeed(.5);
		gyroDrive->straightDrive();
