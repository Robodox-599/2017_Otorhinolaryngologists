...
Autonomous::Autonomous(Gears* autoGear, GyroDrive* autoTurning, AutoDrive* precisionDrive, VisionTracking* vt, Drive* dr, Joystick* joy)
{
...
void Autonomous::linearAuto3()
{
	...
	while(!gear->trapDoor())
	{
		...
		{
			drive->drive(0,0);
			drive->addForwardSpeed(-.25);//change not uploaded
			gyroDrive->straightDrive();
			drive->updateAllMotors();
