...
bool GyroDrive::straightDrive()
{
	...
	{
		integrator += 0.000;//added in an integrator

		if(gDrive->abs(calculateError())*.08 < .13)//tune value
		{
		...
		gDrive->addTurnSpeed(gDrive->sign(calculateError()) * (.13 + integrator));//tune value
